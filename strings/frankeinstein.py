...
# It is a problem from previous year's TCS CODEVITA examinations
...
def frank():
    ...
    bestcase = defaultdict()
    def bestcheck(potion):
        if potion not in recipes:
            return 0
        if potion in bestcase: return bestcase[potion]
        
        best = float('inf') 
        for ingrdients_list in recipes[potion]:
            cost = len(ingrdients_list)-1
            for ing in ingrdients_list:
                cost += bestcheck(ing)
            best = min(cost,best)
        bestcase[potion] = best

        return best
    print(bestcheck(target))
